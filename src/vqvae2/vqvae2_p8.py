            ...
            if not lls:
                blocks.extend(
                    [
                        ...
                        nn.ReLU(inplace=True),
                        nn.ConvTranspose2d(channel // 2, out_channel, 8, stride=2, padding=3),
                    ]
                )
            else:
                blocks.extend(
                    [
                        ...
                        nn.ConvTranspose2d(channel // 2, channel // 2, 4, stride=2, padding=1),
                        nn.ReLU(inplace=True),
                        nn.ConvTranspose2d(channel // 2, 30, 8, stride=2, padding=3),
                    ]
                )
...
class IndependentDecoder(nn.Module):
    def __init__(
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(512, 2048),
            nn.ReLU(),
            nn.Linear(2048, 8 * 8 * self.discret_feats)
        )
    ...
    def forward(self, input):
        ...
        x = self.net(x)
        return x.reshape(B, H, W, self.discret_feats, 8, 8).permute(0, 3, 1, 4, 2, 5).reshape(B, self.discret_feats, H * 8, W * 8)
...
class VQVAE2(nn.Module):
    ...
    def decode(self, quant_t, quant_b):
        repeats = 2*torch.ones(size=(quant_t.shape[2],),dtype=torch.int).to(quant_t.device).long()
