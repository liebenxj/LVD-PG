...
class PixelCNNLayer_down(nn.Module):
    def __init__(self, nr_resnet, nr_filters, resnet_nonlinearity, additional_input_size=0):
        super(PixelCNNLayer_down, self).__init__()
        self.nr_resnet = nr_resnet
        self.additional_input_size = additional_input_size
        # stream from pixels above
        self.u_stream  = nn.ModuleList([
            gated_resnet(nr_filters, down_shifted_conv2d, resnet_nonlinearity, skip_connection=1, additional_input_size=(
                additional_input_size if i == 0 else 0
            )) for i in range(nr_resnet)
        ])
        
        # stream from pixels above and to thes left
        self.ul_stream = nn.ModuleList([
            gated_resnet(nr_filters, down_right_shifted_conv2d, resnet_nonlinearity, skip_connection=2, additional_input_size=(
                additional_input_size if i == 0 else 0
            )) for i in range(nr_resnet)
        ])

    def forward(self, u, ul, u_list, ul_list, context = None):
        for i in range(self.nr_resnet):
            if i == 0 and self.additional_input_size > 0:
                u = torch.cat(
                    (u, context[:,:,None,None].repeat(1,1,u.size(2),u.size(3))), axis = 1
                )
                ul = torch.cat(
                    (ul, context[:,:,None,None].repeat(1,1,u.size(2),u.size(3))), axis = 1
                )
...
class ContextualPixelCNN(nn.Module):
    def __init__(self, nr_resnet=5, nr_filters=80, nr_logistic_mix=10, 
                 resnet_nonlinearity='concat_elu', input_channels=3, 
                 context_channels=256, c_nlatents=256, n_downscale=2, n_levels=3):
        ...
        self.context_enc = nn.Sequential(
            nn.Linear(context_channels, c_nlatents),
            nn.ReLU(),
            nn.Linear(c_nlatents, c_nlatents)
        )
        ...
        self.u_init = down_shifted_conv2d(nr_filters + c_nlatents, nr_filters, filter_size=(2,3), 
                        shift_output_down=True)

        self.ul_init = nn.ModuleList([down_shifted_conv2d(nr_filters + c_nlatents, nr_filters, 
                                            filter_size=(1,3), shift_output_down=True), 
                                       down_right_shifted_conv2d(nr_filters + c_nlatents, nr_filters, 
                                            filter_size=(2,1), shift_output_right=True)])
    ...
    def encode_x(self, x, context):
        ...
        context = self.context_enc(context)
