...
struct CuCondBitsProbCircuit{BitsNodes <: BitsNode}
    ...
    # mapping from param id to edge id (w.r.t. upward edges)
...
function CuCondBitsProbCircuit(pc::ProbCircuit)
    ...
    CuCondBitsProbCircuit{BitsNodes}(bpc, edge2param, param2edge, param2group, pargroup_sizes, num_param_groups,
...
num_parameters(cbpc::CuCondBitsProbCircuit) = 
    length(cbpc.param2edge) + cbpc.num_input_params

import ProbabilisticCircuits: num_nodes, num_edges # extend

num_nodes(cbpc::CuCondBitsProbCircuit) = length(cbpc.bpc.nodes)

num_edges(cbpc::CuCondBitsProbCircuit) = length(cbpc.bpc.edge_layers_up.vectors)

num_input_params(cbpc::CuCondBitsProbCircuit) = cbpc.num_input_params

num_inner_params(cbpc::CuCondBitsProbCircuit) = 
    num_parameters(cbpc) - num_input_params(cbpc)

function mark_nodes(cbpc::CuCondBitsProbCircuit, marked_pcs::Matrix{<:ProbCircuit})
    nodes_map = cbpc.bpc.nodes_map
