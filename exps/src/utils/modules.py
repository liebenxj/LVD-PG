...
# Data-Dependent Initialization + Weight Normalization extension of a "Conv2D" module of PyTorch
class WnConvTranspose2d(WnModule):
    def __init__(self, in_dim, out_dim, kernel_size, stride, padding, output_padding = 0, init_scale=1.0, loggain=True, bias=True):
        super().__init__()
        self.in_dim, self.out_dim, self.kernel_size, self.stride, self.padding, self.output_padding = in_dim, out_dim, kernel_size, stride, padding, output_padding
        self.bias = bias
        self.init_scale = init_scale
        self.loggain = loggain
    ...
    def _init(self, x):
        # calculate unnormalized activations
        y_bchw = self._forward(x)
        assert len(y_bchw.shape) == 4 and y_bchw.shape[:2] == (x.shape[0], self.in_dim)

        # set g and b so that activations are normalized
        y_c = y_bchw.transpose(0, 1).reshape(self.in_dim, -1)
        m = y_c.mean(dim=1)
        s = self.init_scale / (y_c.std(dim=1) + _SMALL)
        assert m.shape == s.shape == self.gain.shape == self.b.shape
    ...
    def _forward(self, x):
        if self.loggain:
            ...
            g = self.gain
        vnorm = self.v.view(self.in_dim, self.out_dim, -1).permute(1, 0, 2).reshape(self.out_dim, -1).norm(p=2, dim=1)
        assert vnorm.shape == self.gain.shape == self.b.shape
        w = self.v * (g / (vnorm + _SMALL)).view(1, self.out_dim, 1, 1)
        return F.conv_transpose2d(x, w, self.b, stride=self.stride, padding=self.padding, output_padding=self.output_padding)
...
class Reshape(nn.Module):
    ...
    def forward(self, x):
        return x.reshape(x.size(0), *self.target_shape)
...
class Permute(nn.Module):
    def __init__(self, permute_target):
        super(Permute, self).__init__()

        self.permute_target = permute_target

    def forward(self, x):
        return x.permute(self.permute_target)
