...
class gated_resnet(nn.Module):
    def __init__(self, num_filters, conv_op, nonlinearity=concat_elu, skip_connection=0, additional_input_size=0):
        super(gated_resnet, self).__init__()
        self.skip_connection = skip_connection
        self.num_filters = num_filters
        self.additional_input_size = additional_input_size
        self.nonlinearity = nonlinearity
        self.conv_input = conv_op(2 * (num_filters + additional_input_size), num_filters) # cuz of concat elu
        
        if skip_connection != 0 : # a 1x1 conv layer
    ...
    def forward(self, og_x, a=None):
        ...
        return og_x[:,:self.num_filters,:,:] + c3
