...
function scale_flows_kernel(flows, flows_buf, edges, _mars, fscale::Float32,
    ...
    if ex_id <= num_examples
        ...
        for edge_id = edge_start : edge_end
            ...
            if issum
                child_prob = mars[ex_id, edge.prime_id] + edge.logp
                if edge.sub_id != 0
                    child_prob += mars[ex_id, edge.sub_id]
                end
                par_prob = mars[ex_id, edge.parent_id]
                scaled_flow = (child_prob - par_prob) / fscale

                if isfirstedge || (edge_id == edge_start)
                    acc = scaled_flow
                else
                    acc = PCs.logsumexp(acc, scaled_flow)
                end

                if islastedge || (edge_id == edge_end)
                    pid = edge.parent_id
                    if islastedge && owned_node
                        flows_buf[ex_id, pid] = acc
                    else
                        CUDA.@atomic flows_buf[ex_id, pid] = PCs.logsumexp(flows_buf[ex_id, pid], acc)
                    end
                end
            end
        end
...
function scale_flows(flows, flows_buf, bpc, mars, layer_start, layer_end, num_examples, fscale; 
                     mine, maxe, debug=false)
    edges = bpc.edge_layers_up.vectors
    num_edges = layer_end-layer_start+1
    dummy_args = (flows, flows_buf, edges, mars, Float32(fscale),
                  Int32(32), Int32(num_examples), 
                  Int32(1), Int32(1), Int32(2))
    kernel = @cuda name="scale_flows" launch=false scale_flows_kernel(dummy_args...) 
    config = launch_configuration(kernel.fun)
    ...
    args = (flows, flows_buf, edges, mars, Float32(fscale),
...
function reg_layer_down_kernel(flows, flows_buf, edge_aggr, edges, _mars, example_ids, fscale::Float32, 
                               num_ex_threads::Int32, num_examples::Int32, 
    ...
    for edge_id = edge_start : edge_end
        ...
        if active
            ...
            if issum
                ...
                scaled_flow = (child_prob - parent_mar) / fscale
                norm_val = flows_buf[ex_id, parent_id]

                edge_flow = edge_flow * exp(scaled_flow - norm_val)
            end
...
function reg_layer_down(flows, flows_buf, edge_aggr, bpc, mars, 
                        layer_start, layer_end, example_ids, weights, fscale; 
                        mine, maxe, debug=false)
    edges = bpc.edge_layers_down.vectors
    num_edges = layer_end-layer_start+1
    num_examples = length(example_ids)
    dummy_args = (flows, flows_buf, edge_aggr, edges, mars, example_ids, Float32(fscale),
                  Int32(32), Int32(num_examples), 
    ...
    args = (flows, flows_buf, edge_aggr, edges, mars, example_ids, Float32(fscale),
            Int32(num_example_threads), Int32(num_examples), 
...
function reg_flows_circuit(flows, flows_buf, edge_aggr, bpc, mars, example_ids, weights; mine, maxe, fscale = 1.0, debug = false)
    num_examples = length(example_ids)
    init_flows() = begin 
        flows .= zero(Float32)
        flows[:,end] .= one(Float32)
        flows_buf .= zero(Float32)
    end
    if debug
    ...
    CUDA.@sync scale_flows(flows, flows_buf, bpc, mars, 1, length(bpc.edge_layers_up.vectors), 
                num_examples, fscale; mine, maxe, debug=false)

    layer_start = 1
    for layer_end in bpc.edge_layers_down.ends
        reg_layer_down(flows, flows_buf, edge_aggr, bpc, mars,
                       layer_start, layer_end, example_ids, weights, fscale; 
                       mine, maxe, debug)
...
function scaled_probs_flows_circuit(flows, flows_buf, mars, edge_aggr, bpc, data, example_ids; mine, maxe, soft_reg, soft_reg_width, 
                                    fscale = 1.0, debug = false, weights = nothing)
    eval_circuit_with_reg(mars, bpc, data, example_ids; mine, maxe, soft_reg, soft_reg_width, debug)
    reg_flows_circuit(flows, flows_buf, edge_aggr, bpc, mars, example_ids, weights; mine, maxe, fscale, debug)
